...
$('.model').on('change', function (e, params) {   //当选项改变的时候
    if (params.selected !== undefined) {  //如果选人值不为空 进入if   params.selected的值就是
        // console.log(params.selected)     params.selected和params.deselected分别是每次添加或删除的值
        arr_t.push(params.selected)   //追加到arr_t数组中  与后面的数组remove方法相结合 使得arr_t数组中存放的就是所有模块被选中的总值
    }
    if (params.deselected !== undefined) { //如果删除的人存在的话 进入if
        // console.log(params.deselected)  //测试
        arr_t.remove(params.deselected)   //就把这个删除的人从arr_t数组中剔除
        $(".model").find("option[class~=" + params.deselected + "]").removeClass("myselected") //如果用户删除或取消某个选人 就让其重新返回备选列表中
    }
    // console.log(arr_t)
